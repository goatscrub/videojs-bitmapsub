...
class Bitmapsub extends Plugin {
  ...
  constructor(player, options) {
    ...
    // tracklist.addEventListener('change', e=>console.log('change'))
    // tracklist.addEventListener('removetrack', e=>console.log('remove'))

    tracklist.addEventListener('addtrack', e => {
      if (e.track.kind === 'metadata' && e.track.label.startsWith('bitmap:')) {
        this.handleBitmapTrack(e.track);
      }
    });
  }

  /** */
  handleBitmapTrack(track) {
    // console.log(track)
    ...
    // this.player.controlBar.subsCapsButton.menu.addChild(new VjsSubsCapsMenuItem(
    //   this.player, { track:{ label:'lalala', language: 'fre', id:'my_id_lalala', default: 'false', kind:'subtitles' } }
    // ))
    // console.log(this.player.controlBar.subsCapsButton.menu.children())
