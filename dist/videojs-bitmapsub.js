...
(function (global, factory) {
  ...
  class BitmapSubtitle extends VjsPlugin {
    ...
    */
    constructor(player, options) {
      ...
      this.player.on('loadeddata', e => {
        ...
        this.updateBitmapMenu(); // Needed at startup, because, underlying this.currentSubtitle is not loaded yet
      ...
      this.player.on('playerresize', this.scaleBmpSubtitleContainer.bind(this)); // Save control bar height at first play, before height is 0
    ...
    appendComponent() {
      ...
      this.subtitleElement = this.bmpSubContainer.el().querySelector(`#${this.player.id()} .bitmap-subtitle`); // Append bitmap menu into video.js controlbar

      this.bitmapMenu = new BitmapMenuButton(this.player, {
        name: 'bitmapMenuButton'
      });
      this.bitmapMenu.addClass('vjs-subtitles-button'); // Place bitmapMenuButton after SubsCapsMenuButton

      const bitmapMenuButtonPlacement = this.player.controlBar.children().indexOf(this.player.controlBar.getChild('SubsCapsButton')) + 1;
    ...
    */
    ...
    menuControlItems() {
      ...
      offBitmap.handleClick = () => {
        const menu = this.player.controlBar.getChild('BitmapMenuButton').menu; // Deselect all items from bitmap menu items
        ...
        offBitmap.addClass('vjs-selected'); // Hide bitmap subtitle container

        this.bmpSubContainer.hide(); // Remove handler on this.currentSubtitle.track
    ...
    */
    ...
    deselectMenuItem() {
      this.player.controlBar.getChild('bitmapMenuButton').menu.children().forEach(e => e.removeClass('vjs-selected'));
